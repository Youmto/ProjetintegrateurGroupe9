...
def supprimer_produit_dans_colis(conn, id_colis, id_produit):
    try:
        # Trouver idLot correspondant au produit et colis dans CONTENIR
        query_lot = """
            SELECT idLot FROM CONTENIR
            WHERE idColis = %s AND idLot IN (
                SELECT idLot FROM LOT WHERE idProduit = %s
            )
            LIMIT 1
        """
